...
trait SimpleVisitor[-T, +V] extends Visitor[T, V]:
  ...
  def visitTrue(index: Int): V =
    throw JsonReadException(expectedMsg + " got boolean", index)
  ...
  def visitFloat64(d: Double, index: Int): V =
    throw JsonReadException(expectedMsg + " got float64", index)

  def visitFloat32(d: Float, index: Int): V =
    throw JsonReadException(expectedMsg + " got float32", index)

  def visitInt32(i: Int, index: Int): V =
    throw JsonReadException(expectedMsg + " got int32", index)

  def visitInt64(i: Long, index: Int): V =
    throw JsonReadException(expectedMsg + " got int64", index)

  def visitUInt64(i: Long, index: Int): V =
    throw JsonReadException(expectedMsg + " got uint64", index)

  def visitFloat64String(s: String, index: Int): V =
    throw JsonReadException(expectedMsg + " got float64 string", index)

  def visitChar(s: Char, index: Int): V =
    throw JsonReadException(expectedMsg + " got char", index)

  def visitBinary(bytes: Array[Byte], offset: Int, len: Int, index: Int): V =
    throw JsonReadException(expectedMsg + " got binary", index)
...
object NoOpVisitor extends Visitor[sd.PValue, sd.PValue] {
  ...
  def visitObject(length: Int, jsonableKeys: Boolean, index: Int) =
    new ObjVisitor[sd.PValue, sd.PValue] {
      def subVisitor = NoOpVisitor.this
